...
def getConfusionMatrixWithSequences(results, expectations):
    """
    ...
    for i in range(len(results)):
        ...
        e = 0
        while r < len(results[i]) or (i < len(expectations) and e < len(expectations[i])):
            if i >= len(expectations) or e >= len(expectations[i]) or (r < len(results[i]) and results[i][r][0] < expectations[i][e][0][0]):
                if results[i][r][1]:
                    FP += 1.0
                else:
                    TN += 1.0
            elif r >= len(results[i]) or (e < len(expectations[i]) and results[i][r][0] > expectations[i][e][0][1]):
                FN += 1.0
                r -= 1
                e += 1
            else:
...
def computeROCCurve(sweepList, expectations, nbThresholds=100, positiveOnly=False, findOnePerSweep=False, sequenced=False):
    ...
    for threshold in thresholdList:
        results = computeResults(sweepList, threshold, positiveOnly, findOnePerSweep)
        if sequenced:
...
def showROCCurve(AUC, pivot, figure=None, plotpos=[1, 1, 1], title="Receiver Operating Characteristic"):
    ...
    if title:
        ax.set_title(title + "\n(AUC=" + str(round(AUC, 4)) + ")")
    ax.plot(np.array(1-pivot.y), np.array(pivot.x))
    ax.plot([0, 1],'r--')
