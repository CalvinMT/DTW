...
def runSearch(queryPath, searchPatternPath="", searchPathList=None, splitList=None):
    """
    ...
    for i, searchFile in enumerate(searchFileList):
        ...
        sweep = []
        for splitMillBorders in splitList[i]:
            splitSecBegin = int(splitMillBorders[0] / 1000)
            splitSecEnd = int(splitMillBorders[1] / 1000)
            splitFrameBegin = sRate * splitSecBegin
            splitFrameEnd = sRate * splitSecEnd
            splitSearch = search[splitFrameBegin:splitFrameEnd]
            searchMFCC = getMFCC(sRate, splitSearch)
            for j in qbe.dtw_sweep(queryMFCC, searchMFCC, 3): # x: time/3, y: cost
                sweep.append(j)
