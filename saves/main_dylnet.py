...
# XXX - splitList
def buildExpectations(query, searchPatternPath="", searchPathList=None, sweepStep=3, inSequence=False):
    """
    Based on DyLNet directory structure.
    """
    expectations = []
...
def job(query, nbThresholds=1000, oneWord=False, inSequence=True, split=100):
    ...
    _, sweepList, _ = dtw.runSearch(queryPath, searchPathList=searchList, splitList=querySplitList)

    # XXX - splitList
    expectations = buildExpectations(query, searchPathList=searchList, inSequence=inSequence)
